...
class Camera: NSObject {
    ...
    var result: (identifier: String, confidence: Int) = ("", 0)
    ...
    private var allCaptureDevices: [AVCaptureDevice] {
        AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInTrueDepthCamera, .builtInDualCamera, .builtInDualWideCamera, .builtInWideAngleCamera, .builtInUltraWideCamera], mediaType: .video, position: .back).devices
    }
...
extension Camera: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        let request = VNCoreMLRequest(model: vnMLModel) { //An image analysis request that uses a Core ML model to process images.
            ...
            for result in results.prefix(1) {
                self.result = (result.identifier, Int(result.confidence * 100))
            }
        }
